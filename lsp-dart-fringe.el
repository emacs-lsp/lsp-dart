;;; lsp-dart-fringe.el --- Dart fringe tools -*- lexical-binding: t; -*-
;;
;; Version: 1.4
...
(defcustom lsp-dart-fringe-colors t
  ...
  :type 'boolean
  :group 'lsp-dart-fringe)

(define-fringe-bitmap 'lsp-dart-fringe-color-bitmap
  [255 255 255 255 255 255 255 255 255])

(defun lsp-dart-fringe--create-face (hex)
  "Create a face for HEX color."
  `((t :foreground ,hex
       :weight bold)))

(defun lsp-dart-fringe--add-color (hex buffer point)
  "Add color HEX overlay to line at POINT from BUFFER."
  (let ((face-name (make-symbol (concat "lsp-dart-fringe-colors-face-" hex)))
        (ov (make-overlay point point buffer t t)))
    (defface face-name (lsp-dart-fringe--create-face hex)
      (format "Fringe color face %s." hex)
      :group 'lsp-dart-fringe)
    (face-spec-set face-name (lsp-dart-fringe--create-face hex))
    (overlay-put ov 'lsp-dart-fringe-colors t)
    (overlay-put ov 'priority 1)
    (overlay-put ov 'before-string
                 (propertize "." 'display `((left-fringe
                                             lsp-dart-fringe-color-bitmap
                                             ,face-name))))))

(defun lsp-dart-fringe--rgb-to-hex (r g b)
...
(defun lsp-dart-fringe--update-colors (buffer)
...
(defvar-local lsp-dart-fringe-colors-timer nil)

(defun lsp-dart-fringe--change-colors-handler (&rest _rest)
  "Handler that add/update colors on fringe."
  (when lsp-dart-fringe-colors-timer
    (cancel-timer lsp-dart-fringe-colors-timer))
  (setq lsp-dart-fringe-colors-timer
        (run-with-idle-timer 0.3 nil #'lsp-dart-fringe--update-colors (current-buffer))))

(define-minor-mode lsp-dart-fringe-colors-mode
...
(provide 'lsp-dart-fringe)
;;; lsp-dart-fringe.el ends here
